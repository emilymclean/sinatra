...
class FavouritePersistence(
    ...
    private fun validate(type: FavouriteType, routeId: RouteId?, stopId: StopId?, placeId: PlaceId?) {
        when (type) {
            FavouriteType.ROUTE -> if (routeId == null || stopId != null || placeId != null)
                throw IllegalArgumentException("A route must specify route ID and must not specify stop ID")
            FavouriteType.STOP -> if (routeId != null || stopId == null || placeId != null)
                throw IllegalArgumentException("A stop must not specify route ID and must specify stop ID")
            FavouriteType.STOP_ON_ROUTE -> if (routeId == null || stopId == null || placeId != null)
                throw IllegalArgumentException("A stop on route must specify both a route ID and a stop ID")
            FavouriteType.PLACE -> if (routeId != null || stopId != null || placeId == null)
                throw IllegalArgumentException("A place must specify a placeId and neither a route ID or a stop ID")
        }
    }

    suspend fun add(type: FavouriteType, routeId: RouteId? = null, stopId: StopId? = null, placeId: PlaceId? = null) {
    ...
    suspend fun add(favourite: Favourite) {
        add(FavouriteType.fromFavourite(favourite), favourite.routeId, favourite.stopId, favourite.placeId)
    }
        ...
        return favouriteDao.get().map {
            it.sortedByDescending { it.favourite.id }.mapNotNull {
                ...
                when (type) {
                    ...
                    FavouriteType.PLACE -> it.place?.let { Favourite.Place(it.toModel()) }
                }
            }
        }
    }

    fun exists(type: FavouriteType, routeId: RouteId? = null, stopId: StopId? = null, placeId: PlaceId? = null): Flow<Boolean> {
        validate(type, routeId, stopId, placeId)
        return when (type) {
